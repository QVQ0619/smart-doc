...
本技能随会话下发三个纯标准库脚本(`scripts/smart_doc_add.py` 上传入库、
`scripts/smart_doc_segments.py` 读段落、`scripts/smart_doc_extract_rules.py` 入库规则)，
**一条龙**把规则文件 上传→识别→抽取(依据条款 + 审查规则)一次走完
(后端原子入 `regulation_clause` + `review_rule`，自动关联出处)。
后端地址在 `scripts/api_base.txt`、密钥在 `scripts/api_key.txt`(前端注入)。

## 何时触发
用户表达「抽取规则 / 抽取审查规则 / 把这个文档的规则抽出来」。目标可以是：
- **刚通过"+"上传到工作区的新文件** —— 本技能会自动先上传入库+识别再抽取，**无需用户先说"保存到规则库"**；
- 或规则库里已识别的文档(书名号标题、doc_code、上文刚处理的文档)。

**一条龙同时产出依据条款和审查规则，不要再让用户单独"保存"或"结构化"。**
...
1. 定位脚本与后端地址/密钥：
   ...
   ADD="$DIR/smart_doc_add.py"
   ```
1.5. **(仅当目标是用户刚通过"+"上传到工作区、规则库里还没有的新文件;已是库内文档则跳过本步)** 先上传入库(后端自动识别)：
   ```bash
   SMART_DOC_API="$API" SMART_DOC_API_KEY="$KEY" python3 "$ADD" "<工作区文件绝对路径>"
   ```
   按 stdout / 退出码处理：
   - `status=created`(码0):新建成功,记下首行 `doc_code`,转第 2 步。
   - `status=reused`(码0):库里已有相同文件,复用该 `doc_code`,转第 2 步。
   - `CONFLICT ... existing_doc_code=.. existing_title=..`(**码 8**):库里已有**同名但内容不同**的文档(更新版冲突)。
     **必须停下来问用户**:「规则库已有《<existing_title>》,这次是**更新**它,还是**另存**为新文档?」
     - 选**更新** → `SMART_DOC_API="$API" SMART_DOC_API_KEY="$KEY" python3 "$ADD" --force --replace "<文件路径>"`(软删旧版+新建)。
     - 选**另存** → `SMART_DOC_API="$API" SMART_DOC_API_KEY="$KEY" python3 "$ADD" --force "<文件路径>"`(保留旧版+新建)。
     拿到新 `doc_code` 后转第 2 步。
   - 码 3/4/6:后端不可达/错误/地址缺失,如实告知,不谎报成功。
2. 读取目标文档段落(参数可为 doc_id / doc_code(SD-开头) / 标题子串;新上传的用上一步的 doc_code)：
   ```bash
